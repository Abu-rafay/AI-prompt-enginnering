...
class BatAndBallGame:
    def __init__(self):
        ...
        cx, cy = 400, 300
        self.btn_start = Button(cx, cy + 180, 200, 50, "START GAME", (0, 255, 255))
    ...
    def _draw_start_screen(self):
        ...
        t_surf = title_font.render("Bat and ball", True, (0, 255, 255))
        self.screen.blit(t_surf, t_surf.get_rect(center=(400, 80)))

        # Fancy Instruction Block
        box_rect = pygame.Rect(150, 200, 500, 180)
        pygame.draw.rect(self.screen, (30, 30, 50), box_rect, border_radius=15)
        pygame.draw.rect(self.screen, (0, 255, 255), box_rect, width=2, border_radius=15)
        
        instr_font = pygame.font.SysFont("Arial", 22, bold=True)
        lines = [
            "HOW TO PLAY",
            "--------------------------",
            "• Move bat with MOUSE or ARROW KEYS",
            "• Destroy all bricks to win",
            "• Keep the ball on screen!",
            "• 5 Ball maximum spawn limit"
        ]
        for i, text in enumerate(lines):
            color = (0, 255, 255) if i == 0 else (200, 200, 200)
            i_surf = instr_font.render(text, True, color)
            self.screen.blit(i_surf, i_surf.get_rect(center=(400, 230 + (i*25))))

    def _overlay_static(self, title, subtitle):
        overlay = pygame.Surface((800, 600), pygame.SRCALPHA)
        overlay.fill((10, 10, 20, 210))
        self.screen.blit(overlay, (0,0))
        t_font = pygame.font.SysFont("Arial", 72, bold=True)
        s_font = pygame.font.SysFont("Arial", 28)
        ts = t_font.render(title, True, (0, 255, 255))
        ss = s_font.render(subtitle, True, (200, 200, 200))
        self.screen.blit(ts, ts.get_rect(center=(400, 230)))
        self.screen.blit(ss, ss.get_rect(center=(400, 310)))
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.state == "START" and self.btn_start.is_clicked(m_pos): self.start_level(1)
                    elif self.state == "LEVEL_COMPLETE" and self.btn_next.is_clicked(m_pos): self.start_level(self.current_level + 1)
            ...
            if self.state == "PLAYING":
                ...
                self.btn_start.update_hover(m_pos)
                ...
                self._overlay_static("GAME OVER", f"You reached Level {self.current_level}")
                self.btn_restart.update_hover(m_pos)
                self.btn_quit.update_hover(m_pos)
                self.btn_restart.draw(self.screen)
                self.btn_quit.draw(self.screen)
            elif self.state == "WIN":
                self._overlay_static("YOU WIN!", "Master of the Neon Grid!")
                self.btn_restart.update_hover(m_pos)
                self.btn_quit.update_hover(m_pos)
                self.btn_restart.draw(self.screen)
                self.btn_quit.draw(self.screen)
