...
pygame.display.set_caption("Tetris - Bonus Score Edition")
...
def main():
    ...
    def reset_game():
        ...
        play_bg_music()
        # Reset state: board, current, next, held, x, y, score, level, lines, game_state, fall_time, can_hold, hard_dropped
        return board, curr_t, SHAPES[curr_t], next_t, SHAPES[next_t], None, 4, 0, 0, 1, 0, "PLAYING", 0, True, False

    board, current_type, current_shape, next_type, next_shape, held_type, piece_x, piece_y, score, level, total_lines, _, fall_time, can_hold, hard_dropped = reset_game()
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if game_state == "START" and start_button_rect.collidepoint(event.pos):
                    board, current_type, current_shape, next_type, next_shape, held_type, piece_x, piece_y, score, level, total_lines, game_state, fall_time, can_hold, hard_dropped = reset_game()
                ...
                if music_button_rect.collidepoint(event.pos):
                    ...
                    current_vol_idx = (current_vol_idx + 1) % len(vol_levels); update_music_volume()
            ...
            if game_state == "PLAYING" and event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    hard_dropped = True # Set flag for bonus calculation
                    fall_time = 1000 
        ...
        if game_state == "PLAYING" and fall_time >= (soft_drop_speed if pygame.key.get_pressed()[pygame.K_DOWN] else current_normal_speed):
            ...
            if not valid_space(current_shape, piece_x, piece_y, board):
                ...
                # --- SCORING LOGIC WITH HARD DROP BONUS ---
                num_cleared = clear_lines(board)
                if num_cleared > 0:
                    base_points = {1: 1, 2: 3, 3: 6, 4: 12}.get(num_cleared, num_cleared * 3)
                    
                    # Apply x2 multiplier if hard dropped
                    multiplier = 2 if hard_dropped else 1
                ...
                piece_x, piece_y = 4, 0
                if not valid_space(current_shape, piece_x, piece_y, board):
                    game_state = "GAME_OVER"; pygame.mixer.music.fadeout(1000)
                    if sounds['gameover']: sounds['gameover'].play()
            fall_time = 0

        # --- RENDERING ---
        draw_fancy_background(screen); draw_animated_title(screen, anim_timer)
        
        if game_state == "START":
            ctrl_box = pygame.Surface((420, 360), pygame.SRCALPHA); pygame.draw.rect(ctrl_box, (255, 255, 255, 20), (0, 0, 420, 360), border_radius=15)
            screen.blit(ctrl_box, (WINDOW_WIDTH // 2 - 210, 180))
            controls = [("CONTROLS", LIGHT_CYAN_GRID), ("Left/Right - Move", WHITE), ("Up Arrow - Rotate CW", WHITE), ("Z Key - Rotate CCW", WHITE), ("Down Arrow - Soft Drop", WHITE), ("Space Bar - Hard Drop (x2 Bonus!)", BRIGHT_GREEN), ("C Key - Hold Piece", WHITE), ("P - Pause / Resume", WHITE)]
            for i, (line, color) in enumerate(controls):
                txt = small_font.render(line, True, color) if i == 0 else tiny_font.render(line, True, color)
                screen.blit(txt, (WINDOW_WIDTH // 2 - txt.get_width() // 2, 195 + (i * 35)))
            
            pulse = math.sin(anim_timer * 4) * 3; btn_rect = start_button_rect.inflate(pulse, pulse)
            ...
            btn_txt = small_font.render("START GAME", True, BLACK); screen.blit(btn_txt, (start_button_rect.centerx - btn_txt.get_width() // 2, start_button_rect.centery - btn_txt.get_height() // 2))
            ...
            hold_label = small_font.render("HOLD", True, WHITE); screen.blit(hold_label, (panel_center - hold_label.get_width() // 2, Y_OFFSET + 10))
            if held_type: draw_tetromino(screen, SHAPES[held_type], SHAPE_COLORS[held_type], 1, 1.5, SIDE_PANEL_X, Y_OFFSET)
            
            next_label = small_font.render("NEXT", True, WHITE); screen.blit(next_label, (panel_center - next_label.get_width() // 2, Y_OFFSET + 110))
            draw_tetromino(screen, next_shape, SHAPE_COLORS[next_type], 1, 5.0, SIDE_PANEL_X, Y_OFFSET)
            
            stats = [("SCORE", score), ("BEST", high_score), ("LEVEL", level), ("LINES", total_lines)]
            for i, (label, val) in enumerate(stats):
                y_pos = Y_OFFSET + 320 + (i * 70)
            ...
            # TOP RIGHT UI
            ui_buttons = [(pause_button_rect, "RESUME" if game_state == "PAUSED" else "PAUSE"), (music_button_rect, f"MUSIC: {int(vol_levels[current_vol_idx]*100)}%")]
            for rect, text in ui_buttons:
                hover = rect.collidepoint(pygame.mouse.get_pos()); pygame.draw.rect(screen, GRAY if hover else PANEL_BG, rect, border_radius=8)
                pygame.draw.rect(screen, LIGHT_CYAN_GRID, rect, 1, border_radius=8); btn_lbl = micro_font.render(text, True, WHITE)
            ...
            if game_state == "PAUSED":
                ...
                screen.blit(overlay, (X_OFFSET, Y_OFFSET)); screen.blit(font.render("PAUSED", True, WHITE), (X_OFFSET + 30, Y_OFFSET + 250))
            if game_state == "GAME_OVER":
                overlay = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SRCALPHA); overlay.fill((0, 0, 0, 180)); screen.blit(overlay, (0, 0))
                go_txt = font.render("GAME OVER", True, RED); restart_txt = small_font.render("Click anywhere to Restart", True, WHITE)
