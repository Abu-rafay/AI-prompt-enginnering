...
def main():
    ...
    while True:
        ...
        # Only progress fall time if playing
        ...
        for event in pygame.event.get():
            ...
            if game_state == "START":
                ...
                # Handle Mouse Pause Button
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if pause_button_rect.collidepoint(event.pos):
                        game_state = "PAUSED" if game_state == "PLAYING" else "PLAYING"

                # Handle Gameplay Controls only if not paused
        ...
        if game_state == "START":
            ...
            pygame.draw.rect(ctrl_box, (255, 255, 255, 50), (0, 0, 400, 300), 2, border_radius=15)
            ...
            controls = [
                ...
                ("P - Pause / Resume", WHITE) # New Control Hint
            ]
            ...
            # Render Gameplay (even if paused)
            ...
            # Show ghost/piece only if not game over
            ...
            if game_state != "GAME_OVER":
                mouse_pos = pygame.mouse.get_pos()
                p_btn_color = GRAY if pause_button_rect.collidepoint(mouse_pos) else PANEL_BG
                pygame.draw.rect(screen, p_btn_color, pause_button_rect, border_radius=8)
                pygame.draw.rect(screen, LIGHT_CYAN_GRID, pause_button_rect, 1, border_radius=8)
                p_text_str = "RESUME" if game_state == "PAUSED" else "PAUSE"
                p_text = tiny_font.render(p_text_str, True, WHITE)
                screen.blit(p_text, (pause_button_rect.centerx - p_text.get_width() // 2, pause_button_rect.centery - p_text.get_height() // 2))
            ...
            if game_state == "PAUSED":
                ...
                pause_msg = font.render("PAUSED", True, WHITE)
                resume_hint = tiny_font.render("Press P to Resume", True, LIGHT_CYAN_GRID)
                screen.blit(pause_msg, (X_OFFSET + (PLAY_WIDTH // 2 - pause_msg.get_width() // 2), Y_OFFSET + 250))
                screen.blit(resume_hint, (X_OFFSET + (PLAY_WIDTH // 2 - resume_hint.get_width() // 2), Y_OFFSET + 310))

            if game_state == "GAME_OVER":
                overlay = pygame.Surface((PLAY_WIDTH, PLAY_HEIGHT)); overlay.set_alpha(180); overlay.fill(BLACK)
                screen.blit(overlay, (X_OFFSET, Y_OFFSET))
                msg = font.render("GAME OVER", True, RED)
                retry = small_font.render("Click anywhere to Restart", True, WHITE)
                screen.blit(msg, (X_OFFSET + (PLAY_WIDTH // 2 - msg.get_width() // 2), Y_OFFSET + 240))
