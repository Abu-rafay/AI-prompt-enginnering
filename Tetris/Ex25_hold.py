...
pygame.display.set_caption("Tetris - Ultimate Hold Edition")
...
def main():
    ...
    def reset_game():
        ...
        play_bg_music()
        return board, curr_t, SHAPES[curr_t], next_t, SHAPES[next_t], None, 4, 0, 0, 1, 0, "PLAYING", 0, True

    # Added held_type, can_hold to main state
    board, current_type, current_shape, next_type, next_shape, held_type, piece_x, piece_y, score, level, total_lines, _, fall_time, can_hold = reset_game()
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if game_state == "START" and start_button_rect.collidepoint(event.pos):
                    board, current_type, current_shape, next_type, next_shape, held_type, piece_x, piece_y, score, level, total_lines, game_state, fall_time, can_hold = reset_game()
            ...
            if game_state == "PLAYING" and event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_c and can_hold:
                    ...
                    piece_x, piece_y = 4, 0
                    can_hold = False # Rule: One hold per drop
        ...
        if game_state == "PLAYING" and fall_time >= (soft_drop_speed if pygame.key.get_pressed()[pygame.K_DOWN] else current_normal_speed):
            ...
            if not valid_space(current_shape, piece_x, piece_y, board):
                ...
                # Piece Locked: Allow holding again
                can_hold = True
                current_type, current_shape = next_type, SHAPES[next_type]
                next_type = random.choice(list(SHAPES.keys())); next_shape = SHAPES[next_type]
                piece_x, piece_y = 4, 0
        ...
        if game_state == "START":
            ctrl_box = pygame.Surface((400, 320), pygame.SRCALPHA)
            pygame.draw.rect(ctrl_box, (255, 255, 255, 20), (0, 0, 400, 320), border_radius=15)
            screen.blit(ctrl_box, (WINDOW_WIDTH // 2 - 200, 190))
            controls = [("CONTROLS", LIGHT_CYAN_GRID), ("Left/Right - Move", WHITE), ("Up Arrow - Rotate", WHITE), ("Down Arrow - Soft Drop", WHITE), ("Space Bar - Hard Drop", WHITE), ("C Key - Hold Piece", BRIGHT_GREEN), ("P - Pause / Resume", WHITE)]
            for i, (line, color) in enumerate(controls):
                txt = small_font.render(line, True, color) if i == 0 else tiny_font.render(line, True, color)
                screen.blit(txt, (WINDOW_WIDTH // 2 - txt.get_width() // 2, 210 + (i * 38)))
            ...
            # --- HOLD PANEL (LEFT SIDE OF BOARD / INSIDE STATS PANEL) ---
            hold_label = small_font.render("HOLD", True, WHITE)
            screen.blit(hold_label, (panel_center - hold_label.get_width() // 2, Y_OFFSET + 10))
            if held_type:
                # Draw held piece at top of side panel, shift stats down
                draw_tetromino(screen, SHAPES[held_type], SHAPE_COLORS[held_type], 1, 1.5, SIDE_PANEL_X, Y_OFFSET)
            
            # Next Label shifted down
            next_label = small_font.render("NEXT", True, WHITE)
            screen.blit(next_label, (panel_center - next_label.get_width() // 2, Y_OFFSET + 110))
