...
# RESTORED FONT SETTINGS
font = pygame.font.SysFont(None, 60)

board = [[(0,0,0) for _ in range(10)] for _ in range(20)]

def clear_lines(board):
    """Checks for full rows, removes them, and shifts everything down."""
    lines_cleared = 0
    # Iterate backwards through the board (bottom to top)
    for r in range(len(board) - 1, -1, -1):
        row = board[r]
        # If there is no black (0,0,0) in the row, it's full
        if (0, 0, 0) not in row:
            lines_cleared += 1
            # Remove the row
            del board[r]
            # Insert a new empty row at the very top (index 0)
    ...
    while True:
        ...
        if fall_time >= fall_speed:
            ...
            if not valid_space(current_piece, piece_x, piece_y, board):
                ...
                # Check for line clears right after locking
                clear_lines(board)
        ...
        draw_tetromino(screen, next_piece, 1, 2, SIDE_PANEL_X, Y_OFFSET)
