...
pygame.display.set_caption("Tetris - Lock & Spawn System")
font = pygame.font.SysFont(None, 60)

# 1. Initialize the Board (20 rows x 10 columns)
# Storing (0,0,0) for empty or an RGB tuple for a locked block
board = [[(0,0,0) for _ in range(10)] for _ in range(20)]

def valid_space(shape_type, grid_x, grid_y, board):
    """Checks if the piece's current position is valid (not overlapping or out of bounds)."""
        ...
        for c, cell in enumerate(row):
            if cell:
                ...
                if new_x < 0 or new_x >= 10 or new_y >= 20:
                    return False
                # Collision with locked blocks check
...
def draw_board(surface, board):
    """Draws the locked blocks stored in the board array."""
    for r in range(20):
        for c in range(10):
            if board[r][c] != (0,0,0):
...
def main():
    ...
    while True:
        ...
        if fall_time >= fall_speed:
            piece_y += 1
            if not valid_space(current_piece, piece_x, piece_y, board):
                # If moving down is invalid, lock it!
                piece_y -= 1
