...
def main():
    ...
    while True:
        ...
        fall_speed = SOFT_DROP_SPEED if keys[pygame.K_DOWN] else NORMAL_SPEED
        
        # Track if the current lock-in was caused by a hard drop
        was_hard_drop = False
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    while valid_space(current_shape, piece_x, piece_y + 1, board):
                        piece_y += 1
                    was_hard_drop = True
                    # Set fall_time to fall_speed to trigger the locking logic immediately below
                    fall_time = fall_speed 

        # Gravity & Locking Logic
        if fall_time >= fall_speed:
            ...
            if not valid_space(current_shape, piece_x, piece_y, board):
                ...
                if num_cleared > 0:
                    ...
                    # Apply x2 multiplier if this lock was triggered by Hard Drop
                    if was_hard_drop:
                        turn_points *= 2
                    score += turn_points
                
                # Reset flags and spawn next
                was_hard_drop = False
        ...
        screen.blit(next_label, (SIDE_PANEL_X + (SIDE_PANEL_WIDTH // 2 - next_label.get_width() // 2), Y_OFFSET + 20))
        ...
        score_value = font.render(str(score), True, LIGHT_CYAN_GRID)
        screen.blit(score_label, (SIDE_PANEL_X + (SIDE_PANEL_WIDTH // 2 - score_label.get_width() // 2), Y_OFFSET + 250))
        screen.blit(score_value, (SIDE_PANEL_X + (SIDE_PANEL_WIDTH // 2 - score_value.get_width() // 2), Y_OFFSET + 300))
