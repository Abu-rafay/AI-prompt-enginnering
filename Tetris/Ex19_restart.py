...
pygame.display.set_caption("Tetris - Restart Feature")
...
def main():
    ...
    def reset_game():
        ...
        return board, current_type, current_shape, next_type, next_shape, 4, 0, 0, 1, 0, False, 0
    ...
    while True:
        ...
        for i, (label, val) in enumerate([("SCORE", score), ("LEVEL", level), ("LINES", total_lines)]):
            ...
            screen.blit(lbl, (panel_center - lbl.get_width() // 2, Y_OFFSET + 180 + (i * 80)))
            screen.blit(v_lbl, (panel_center - v_lbl.get_width() // 2, Y_OFFSET + 205 + (i * 80)))
        ...
        if game_over:
            ...
            retry_msg = tiny_font.render("Click anywhere", True, WHITE)
            retry_msg2 = tiny_font.render("to Restart", True, WHITE)
            
            screen.blit(msg, (X_OFFSET + (PLAY_WIDTH // 2 - msg.get_width() // 2), Y_OFFSET + 200))
            screen.blit(retry_msg, (X_OFFSET + (PLAY_WIDTH // 2 - retry_msg.get_width() // 2), Y_OFFSET + 260))
            screen.blit(retry_msg2, (X_OFFSET + (PLAY_WIDTH // 2 - retry_msg2.get_width() // 2), Y_OFFSET + 285))
